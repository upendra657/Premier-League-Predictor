...
def _write_metrics(book: Workbook, results: dict) -> None:
    ...
    _note(
        sheet, "A20",
        "Note the tension worth understanding before quoting these: Random Forest + isotonic "
        "wins on Brier, RPS and ECE, yet backtests at -18.5% on held-out seasons while "
        "uncalibrated XGBoost returns +1.5%. Aggregate calibration is measured over all "
        "predictions; betting only samples the tail where the model disagrees with the market. "
        "Isotonic regression is a monotone step function fitted on sparse tail bins, so it "
        "flattens exactly the disagreements the strategy trades on — 279 qualifying bets "
        "versus 877. The backtest, not the scoring rule, picked the served model.",
    )
...
def _write_readme(book: Workbook, results: dict) -> None:
    ...
    provenance = [
        ...
        ("Odds", f"football-data.co.uk 1X2 odds, {data['odds_coverage']:.1%} coverage; "
                 "'best' = maximum across bookmakers"),
        ("Evaluation", "Walk-forward: refit each season, predict the next, never look back"),
        ("Model", "XGBoost, uncalibrated — selected on held-out backtest ROI, NOT on Brier "
                  "(Random Forest + isotonic scores better on Brier, RPS and ECE but "
                  "backtests at -18.5%; see Model Metrics)"),
